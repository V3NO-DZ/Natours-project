...
exports.resizeUserPhoto = catchAsync(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    (req.file as any).filename =
      `user-${(req.user as any).id}-${Date.now()}.jpeg`;

    await sharp(req.file.buffer)
      .resize(500, 500)
      .toFormat('jpeg')
      .jpeg({ quality: 90 })
      .toFile(`public/img/users/${(req.file as any).filename}`);
 ...
 */
exports.getMe = (req: Request, _res: Response, next: NextFunction) => {
  (req.params as any).id = (req.user as any).id;
  next();
};
/**
 * Get a user by ID
...
exports.updateMe = catchAsync(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (req.file) filteredBody.photo = (req.file as any).filename;
