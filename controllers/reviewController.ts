 ...
 */
exports.getAllReviews = catchAsync(async (req: Request, res: Response) => {
  let filter = {};
  if (req.params.tourId) filter = { tour: (req.params as any).tourId } as any;
 ...
 */
exports.createReview = catchAsync(async (req: Request, res: Response) => {
  if (!(req.body as any).tour)
    (req.body as any).tour = (req.params as any).tourId;
  if (!(req.body as any).user) (req.body as any).user = (req.user as any).id;
...
exports.getReview = catchAsync(
  async (req: Request, res: Response, next: NextFunction) => {
    const review = await Review.findById((req.params as any).id).select('-__v');
