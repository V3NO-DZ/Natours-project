...
type DecodedToken = JwtPayload & { id: string; iat: number };
...
const signToken = (id: string) => {
  ...
  if (expiresInEnv)
    (options as unknown as { expiresIn: string }).expiresIn = expiresInEnv;
...
exports.protect = catchAsync(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    const currentUser: any = await User.findById((decoded as DecodedToken).id);
    if (!currentUser) {
      return next(
        new AppError(
    ...
    if (currentUser.changedPasswordAfter((decoded as DecodedToken).iat)) {
      return next(
        new AppError(
 ...
 * @group User
 * @param {string} passwordCurrent.body.required - current password
 * @param {string} password.body.required - new password
 * @param {string} passwordConfirm.body.required - password confirmation
 * @returns {object} 200 - Success message
 */
exports.updatePassword = catchAsync(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    const user: any = await User.findById((req.user as any).id).select(
      '+password'
    );
